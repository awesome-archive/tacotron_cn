...
class WaveNetModel(object):
    ...
    def predict_proba_incremental(self, waveform, global_condition=None,
        ...
        with tf.name_scope(name):
            #encoded = tf.one_hot(waveform, self.skeleton_channels)
            #encoded = tf.reshape(encoded, [-1, self.skeleton_channels])
            #raw_output = self._create_generator(encoded, gc_embedding)
            raw_output = self._create_generator(waveform, global_condition)
